...
wpcr3.ajaxPost = function(parent, data, cb) {
	return jQuery.ajax({
		...
		error : function(rtn) {
			alert('Произошла неизвестная ошибка. E01');
			wpcr3.enableSubmit();
		}
	});
};
...
wpcr3.submit = function(e) {
	...
	if (wpcr3.mousemove_total <= wpcr3.mousemove_need || wpcr3.keypress_total <= wpcr3.keypress_need) {
		alert('Вы не прошли нашу проверку на человека. Код '+wpcr3.mousemove_total+','+wpcr3.keypress_total);
		return false;
	}
	...
	if (c1_fail || c3_fail || fake_fail) {
		alert('Вы не прошли проверку нашего обнаружения бота. Код '+c1_fail+','+c3_fail+','+fake_fail);
		return false;
	}
	if (c2_fail) {
	...
	wpcr3.ajaxPost(parent, ajaxData, function(err, rtn) {
		...
		alert('Спасибо! Ваш отзыв был получен и будут опубликованы в ближайшее время.');
		$(window).scrollTop(0);
		wpcr3.clearFields();
